...
contract FunctionsScript is BaseScript {
  ...
  function createSubscription() nestedScriptContext isAllowListed external returns(uint64) {
  ...
  function createSubscriptionWithConsumer(
    address consumer
  ) nestedScriptContext isAllowListed external returns(uint64) {
    IFunctionsSubscriptions functionsRouter = IFunctionsSubscriptions(functionsRouterAddress);
  ...
  function estimateRequestCost(
    ...
    uint256 gasPriceWei
  ) nestedScriptContext isAllowListed external returns(uint96) {
